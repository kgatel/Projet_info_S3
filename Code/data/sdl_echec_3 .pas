...
var a,r1,r2,t1,t11,t22,t2,i,j : integer;
	...
	compteur_0: Longint;
	echecmat, vf,prise_en_passant0,pat0, wsh1,wsh2 : boolean;
	sauvegarde : text;
	...
	zero, un, deux, trois, quatre, cinq, six, sept, huit, neuf, deuxpoints, timermin, timersec, timerjoueur1, timerjoueur2 : psdl_surface;
	position, other, tailletext, cursor, postimer : tsdl_rect;
...
procedure save(a,r1,r2,t1,t11,t2,t22,compteur: integer);
begin
...
procedure load(var a,r1,r2,t1,t11,t2,t22,compteur:integer);
var chaine : string;
...
procedure affichetimer(var ctm, ctm2,cts, cts2, joueurtimer : integer);
...
postimer.y :=10; 
	postimer.x :=650; 
	sdl_blitsurface(timerjoueur1, nil, window,@postimer); sdl_flip(window);



		postimer.x:=740;
			case ctm of 
			0 : begin sdl_blitsurface(zero, nil, window, @postimer); sdl_flip(window); end;
			1 : begin sdl_blitsurface(un, nil, window, @postimer); sdl_flip(window); end;
			2 : begin sdl_blitsurface(deux, nil, window, @postimer); sdl_flip(window); end;
			3 : begin sdl_blitsurface(trois, nil, window, @postimer); sdl_flip(window); end;
			4 : begin sdl_blitsurface(quatre, nil, window, @postimer); sdl_flip(window); end;
			5 : begin sdl_blitsurface(cinq, nil, window, @postimer); sdl_flip(window); end;
			6 : begin sdl_blitsurface(six, nil, window, @postimer); sdl_flip(window); end;
			7 : begin sdl_blitsurface(sept, nil, window, @postimer); sdl_flip(window); end;
			8 : begin sdl_blitsurface(huit, nil, window, @postimer); sdl_flip(window); end;
			9 : begin sdl_blitsurface(neuf, nil, window, @postimer); sdl_flip(window); end;
			end;
		postimer.x :=760;
		sdl_blitsurface(timermin, nil, window,@postimer); sdl_flip(window);
		postimer.x :=790;
			case cts of 
			0 : begin sdl_blitsurface(zero, nil, window, @postimer); sdl_flip(window); end;
			1 : begin sdl_blitsurface(un, nil, window, @postimer); sdl_flip(window); end;
		...
		postimer.x :=810;
		sdl_blitsurface(timersec, nil, window, @postimer); sdl_flip(window);

postimer.y :=30; 
	postimer.x :=650; 
	sdl_blitsurface(timerjoueur2, nil, window,@postimer); sdl_flip(window);
	...
	for i:=1 to tps_initial1 do
		begin
			if tt-t=i then
				...
				affichetimer(minute, minute2, seconde, seconde2, a);
...
procedure deplacement2(var xtab,ytab,x3tab,y3tab,x4tab,y4tab,x5tab,y5tab,x6tab,y6tab,r1,r2,t1,t11,t2,t22,compteur : integer; p1,p3,p4 : string; var cste,a : integer; var vf,echec,prise_en_passant0 : boolean);
	...
	BEGIN
		...
		SDL_KEYDOWN: BEGIN
                     ...
                     case test_event^.key.keysym.sym of
						...
						27  : loop:=TRUE;
						13  : deplacement2(xtab,ytab,x3tab,y3tab,x4tab,y4tab,x5tab,y5tab,x6tab,y6tab,r1,r2,t1,t11,t2,t22,compteur,p1,p3,p4,cste,a,vf,echec,prise_en_passant0);
						end;
...
begin
	...
	case newload of 
				'new'  : initialisation();
				'load' : load(a,r1,r2,t1,t11,t2,t22,compteur);
	end;
	...
	begin{curseur}
		...
		save(a,r1,r2,t1,t11,t2,t22,compteur);
